	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (String ppName : postProcessorNames) {
				// ConfigurationClassPostProcessor统一实现了PriorityOrdered接口
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					// 这里调用了getBean方法，生产了ConfigurationClassPostProcessor，放到currentRegistryProcessors集合中
			...
			// 扫描bean对象 生成BeanDefinition
			invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry);
			// 清空，以便处理后面的后置处理器
			currentRegistryProcessors.clear();

			// 再次查找实现类BeanDefinitionRegisterPostProcessor接口的BeanName，这里就是从配置类中解析出来的一些
			// Next, invoke the BeanDefinitionRegistryPostProcessors that implement Ordered.
			postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
			for (String ppName : postProcessorNames) {
				// 不包括已经处理过的，并且先处理实现Order接口的
			...
			// 将生产出来的后置处理器放到集合中
			registryProcessors.addAll(currentRegistryProcessors);
			// 调用所有后置处理器的ConfigurationClassPostProcessor的postProcessBeanDefinitionRegistry方法
			invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry);
			// 再次清理，因为后面还要处理未实现Ordered接口的
			currentRegistryProcessors.clear();

			// 循环处理所有剩下的后置处理器，直到全部处理完
			...
			// 如果自己没有实现的话，Spring中只有一个内置的ConfigurationClassProcessor
